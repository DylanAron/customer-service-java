...
- 注释请使用中文注释。
- 优先遵循当前代码结构和已有命名风格。
- 后端修改后优先运行 `mvn clean compile` 验证。
- 前端修改后优先运行 `npm run build` 或 `npm run lint` 验证。
- 不要提交运行时上传文件、日志、构建产物或本地 IDE 配置。
- `uploads/` 是旧上传目录，新的文件存储逻辑使用 `filesUpload/`。
- 文件存储业务代码应依赖 `FileStorageService` 接口，避免直接绑定本地文件系统实现。
...
访客用户无需登录，直接访问 `/` 即可匿名聊天，浏览器会自动生成 `userId`。
...
- `text`：文本消息。
- `image`：图片消息，含 `fileUrl`。
- `file`：文件消息，含 `fileUrl`。

### 客服分配策略（Redis 版）

1. 检查用户是否已有分配且客服在线，若是则复用。
2. 否则从在线且启用的客服中随机选择。
3. 分配写入 Redis，TTL 为 1 天。
4. 分配在用户发消息时触发延迟绑定。
...
./start.sh restart
./start.sh status
SPRING_PROFILE=dev ./start.sh
JAVA_OPTS="-Xms1g -Xmx2g" ./start.sh
```
...
- JPA 实体注解改为 MyBatis-Plus 注解：`@TableName`、`@TableId`、`@TableField`。
- `JpaRepository` 改为 `BaseMapper` + `LambdaQueryWrapper`。
- 自定义 `@Query` 改为 XML `MessageMapper.xml`。
- DDL 从 `Hibernate ddl-auto` 改为 `schema.sql + spring.sql.init`。
