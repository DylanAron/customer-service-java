...
CREATE TABLE IF NOT EXISTS cs_auto_reply_rule (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    keywords TEXT NOT NULL COMMENT '匹配词JSON数组',
    reply_content TEXT NOT NULL COMMENT '命中后的富文本回复内容',
    enabled TINYINT(1) DEFAULT 1 COMMENT '是否启用',
    priority INT DEFAULT 100 COMMENT '优先级，数字越小越先匹配',
    remark VARCHAR(255) COMMENT '备注',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间'
) DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='关键词自动回复规则表';
